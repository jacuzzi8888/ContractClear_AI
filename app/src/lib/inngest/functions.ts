...
export const analyzeContract = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    // ── Step 7: Auto-Draft Email (Agentic Action) ───────────
    await step.run("agentic-draft-email", async () => {
      ...
      if (auth0SubjectToken) {
        ...
        if (token) {
          ...
          const { createGmailDraft } = await import("../gmail-api");
          
          // Build the email body
          const criticalIssues = extraction.issues.filter((i: any) => i.risk_level === "critical" || i.risk_level === "high");
          
          const emailSubject = `Questions regarding the contract: ${fileName}`;
          let emailBody = `Hello,\n\nI have reviewed the recent contract draft. While I look forward to working together, I have some concerns regarding a few clauses that I would like to clarify or negotiate before signing:\n\n`;

          if (criticalIssues.length > 0) {
            criticalIssues.forEach((issue: any, index: number) => {
              emailBody += `${index + 1}. Regarding page ${issue.page_number} ("${issue.quote}"): \n   ${issue.recommended_action}\n\n`;
            });
          } else {
            emailBody += `Overall, the contract looks reasonable, but I would like to confirm a few minor details... [Draft generated by ContractClear AI]\n\n`;
          }

          emailBody += `Could we schedule a brief call to discuss these points?\n\nBest regards,`;
          ...
          if (draftResult.success) {
            console.log(`[Inngest] Successfully created Gmail draft ${draftResult.draftId}`);
          } else {
            console.error(`[Inngest] Failed to create draft:`, draftResult.error);
          }
        } else {
